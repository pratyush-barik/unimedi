...
export default function DoctorLoginPage() {
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    if (!cleanEmail) {
      setMessage({ text: "Please enter your doctor account email.", type: "error" });
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setOtpSent(true);
        setMessage({
          text: `A 6-digit OTP code has been sent to ${cleanEmail}.`,
          type: "success",
        });
      } else {
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl border border-slate-200/80 p-8 sm:p-10">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Access appointment queue, QR console &amp; Rx builder
          </p>
        </div>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-center text-2xl font-mono font-bold tracking-[6px] text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition"
              />
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col items-center gap-3 text-xs text-slate-500">
          <p className="text-center text-[11px] text-slate-400">
            Doctor accounts are provisioned by hospital administrators.
          </p>
          <Link
            href="/"
